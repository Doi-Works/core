...
class TestNode():
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir, use_cli=False):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("BITCOIND", "doichaind")
        else:
            self.binary = binary
        self.stderr = stderr
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "doichaind exited with status %i during initialization" % self.process.returncode
            try:
        ...
        raise AssertionError("Unable to connect to doichaind")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes doichaind to shutdown, so this method takes

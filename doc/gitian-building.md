...
Clone the git repositories for doichain-core and Gitian.
...
in the doichain-core repository under 'Fetch and create inputs' to install sources which require
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/doichain/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/doichain/doichain-core
...
COMMIT=hardfork-nmc
./bin/gbuild --commit doichain=${COMMIT} --url doichain=${URL} ../doichain-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit doichain=${COMMIT} --url doichain=${URL} ../doichain-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit doichain=${COMMIT} --url doichain=${URL} ../doichain-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../doichain-core/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/doichain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/doichain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/doichain-osx-build.assert
```
...
[doichain/gitian.sigs](https://github.com/doichain/gitian.sigs/) repository, or if that's not possible create a pull
